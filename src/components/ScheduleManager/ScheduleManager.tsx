...
export default function ScheduleManager({
    ...
    const handleSave = async () => {
        if (
            ...
            !editingSchedule.destinationId ||
            !editingSchedule.sourceIds?.length
        ) {
                            ...
                            {schedules.length === 0 ? (
                                ...
                                <div className="space-y-3">
                                    {schedules.map((schedule) => (
                                        <div
                                            key={schedule.id}
                                            className="bg-dark-800 rounded-lg p-4 flex items-center justify-between border border-dark-700"
                                        >
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500 mt-1">
                                                    {
                                                        sources.filter((s) =>
                                                            schedule.sourceIds.includes(s.id)
                                                        ).length
                                                    }{' '}
                                                    source(s) vers{' '}
                                                    {destinations.find(
                                                        (d) => d.id === schedule.destinationId
                            ...
                            <div>
                                ...
                                <div className="space-y-2 max-h-40 overflow-y-auto pr-2 bg-dark-950/50 rounded-lg p-3 border border-dark-800">
                                    {sources.map((source) => (
                                        <label
                                            key={source.id}
                                            className="flex items-center gap-3 p-2 hover:bg-dark-800 rounded cursor-pointer group"
                                        >
                                            <div
                                                className={`
                                                w-5 h-5 rounded border flex items-center justify-center transition-colors
                                                ${
                                                    (editingSchedule.sourceIds || []).includes(
                                                        source.id
                                                    )
                                            ...
                                            <input
                                                type="checkbox"
                                                className="hidden"
                                                checked={(editingSchedule.sourceIds || []).includes(
                                                    source.id
                                                )}
                                                onChange={() => {
